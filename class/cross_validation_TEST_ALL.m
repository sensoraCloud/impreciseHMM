function [ ] = cross_validation_TEST_ALL( n_fold,N,F,M,s,datasetNameIstrogram,datasetNameSnippet,q,alpha,L,multi_features,minimum_value,cmd_Istrogram,cmd_Snippet,normalize)
...
if exist(modelsFileNameImprecise,'file') ~= 2
    %istogram 
    typeFeatures=2;  
    [ modelsPrecise_Istrogram modelsImprecise_Istrogram  ] =learnModels(FileDatasetName_Istogram,modelsFileNamePrecise,modelsFileNameImprecise,N,F,M,1,[],s,typeFeatures);
else
...
if exist(modelsFileNameImprecise,'file') ~= 2
    %snippet 
    typeFeatures=1;
    [ modelsPrecise_Snippet modelsImprecise_Snippet  ] =learnModels(FileDatasetName_Snippet,modelsFileNamePrecise,modelsFileNameImprecise,N,F,M,1,[],s,typeFeatures);
else
...
results{1,2}={'real label' 'likelihood' 'imprecise likelihood' 'Istogrammi: stationary (imprecise) (interval domin)' 'Istogrammi: SVM stationary (precise(Learn SVM with multiple point))' 'Istogrammi: transition (imprecise) (interval domin)' 'Istogrammi: SVM transition (precise(Learn SVM with multiple point))' 'Istogrammi: KNN k=1' 'Istogrammi: SVM precise' 'Snippet: KNN k=1' 'Snippet: SVM precise'};
