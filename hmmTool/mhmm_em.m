function [LL, prior, transmat, mu, Sigma, mixmat,beta,alpha,gamma,xi_summed] = ...
...
while (num_iter <= max_iter) & ~converged
    % E step
    [loglik, exp_num_trans, exp_num_visits1, postmix, m, ip, op,beta,alpha,gamma,xi_summed,gamma2] = ...
    ...
    consistent  = consistentHMM( prior,transmat );
    ...
    if M>1
        ...
        for f=1:O
            for s=1:Q
                for m=1:M
                    gammaTmp(1,:)=gamma2(s,m,:);
                    mu(f,s,m)= ( gammaTmp * data{1}(f,:)') * ( 1 / sum(gammaTmp) );
                end
            end
        end
        ...
        for s=1:Q
            for f=1:O
                for m=1:M
                    gammaTmp(1,:)=gamma2(s,m,:);
                    Sigma(f,f,s,m)= sum( (data{1}(f,:) - mu(f,s,m)).^2 .* gammaTmp ) * 1 / sum(gammaTmp);
                    if Sigma(f,f,s,m)<0.0001,
                        Sigma(f,f,s,m)=0.0001;
                    end;
                end
            end
        end
        ...
        for f=1:O
            for s=1:Q
                mu(f,s,m)= (gamma(s,:) * data{1}(f,:)') * ( 1 / sum(gamma(s,:)) );
            end
        end
        ...
        for s=1:Q
            for f=1:O
                Sigma(f,f,s,m)= sum( (data{1}(f,:) - mu(f,s,m)).^2 .* gamma(s,:) ) * 1 / sum(gamma(s,:));
                if Sigma(f,f,s,m)<0.0001,
                    Sigma(f,f,s,m)=0.0001;
                end;
            end
...
function [loglik, exp_num_trans, exp_num_visits1, postmix, m, ip, op,beta,alpha,gamma,xi,gamma2] = ...
    ess_mhmm(prior, transmat, mixmat, mu, Sigma, data)
